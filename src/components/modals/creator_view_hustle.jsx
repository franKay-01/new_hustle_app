...
export default function CreatorViewHustleDetailModal({handleClose, show, handleCheckApplicant, activeHustleDetails, statusMessage, handleComplete}) {
  ...
  const [activeHustler, setActiveHustler] = useState(false)
  ...
  const { getHustleProposalDetail } = useHustleFunctions();

  const statusLabelMap = {
    applied: 'Created',
    progress: 'In Progress',
    pending_hustler: 'Awaiting hustler approval',
  ...
  const getHustleDetails = async () => {
    setIsLoading(true)
    const {response_code, hustle, msg} = await getHustleProposalDetail(activeHustleDetails.hustle_uuid);

    if (response_code === 200 && hustle.length > 0){
      const hustleData = hustle[0];
      const hasAcceptedBid = hustleData.bids?.some(bid => bid.status === "accepted");
      if (hasAcceptedBid) {
        setActiveHustler(true);
      }
      
      setHustleDetail(hustleData);
      setIsLoading(false)
      return
    }
  ...
  useEffect(() => {
    if (hustleDetail?.preferred_start_time && hustleDetail?.preferred_end_time) {
      const durationValue = getDuration(hustleDetail.preferred_start_time, hustleDetail.preferred_end_time);
      setDuration(durationValue)
    }
  }, [hustleDetail?.preferred_start_time, hustleDetail?.preferred_end_time]);
          ...
          <>
            ...
            <div className="flex flex-col gap-2 p-3">
              <div className="flex flex-col">
                ...
                <h1 className="view-more-header">{hustleDetail.title}</h1>
                <div className="flex flex-row gap-4 mt-2">
                  ...
                  <h1 onClick={() => setJobDescription(false)} className={`${jobDescription ? 'info-card-desc' : 'info-card-desc-mb info-card-desc'} cursor-pointer`}>Applicants ({hustleDetail?.bids?.length})</h1>
                </div>
                ...
                { jobDescription ?
                  <>
                    ...
                    <div className="flex flex-row justify-between mt-4">
                      <div className="flex flex-col">
                        ...
                        <h1 className="info-card-text-color">{hustleDetail?.experience_level ? hustleDetail?.experience_level : "Not specified"}</h1>
                      </div>
                      <div className="flex flex-col">
                        <h1 className="info-card-time info-card-time-alt">Hustle duration</h1>
                        <h1 className="info-card-text-color">{duration === '' ? "...calculating" : duration}</h1>
                      </div>
                      <div className="flex flex-col">
                        <h1 className="info-card-time info-card-time-alt">Amount</h1>
                        <h1 className="info-card-text-color">GHS {hustleDetail?.budget}</h1>
                      </div>
                      <div className="flex flex-col">
                        ...
                        <h1 className="info-card-text-color">
                          {hustleDetail?.preferred_start_time && hustleDetail?.preferred_end_time && (
                            <h1 className="info-card-text-color">
                              {formatTime24To12(hustleDetail.preferred_start_time)} - {formatTime24To12(hustleDetail.preferred_end_time)}
                            </h1>
                          )}
                        </h1>
                      </div>
                      <div className="flex flex-col">
                        ...
                        <h1 className="info-card-text-color">{hustleDetail?.preferred_date}</h1>
                      </div>
                    </div>

                    <h1 className='modal-header-text-alt mt-8'>Skills & expertise</h1>
                    <div className="flex flex-wrap gap-2 mt-2">
                      { hustleDetail.skills_required ? 
                        <>
                          { hustleDetail.skills_required.map((skill, index) => {
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4">
                    { hustleDetail?.bids?.length > 0 ?
                      <>
                        { hustleDetail?.bids?.map((bidder, index) => {
                          return <div key={index} onClick={() => handleCheckApplicant(bidder)} className="info-card cursor-pointer flex flex-row gap-4 items-center px-2 py-4">
                            <img src={ClientImg} alt="" />
                            <div className="flex flex-col">
                              <h1 className="info-card-header">{bidder.hustler.full_name}</h1>
                              <h1 className="info-card-desc">Proposed amount: GHS <b>{bidder.amount}</b></h1>
                              <h1 className="info-card-desc capitalize">Staus: <b>{bidder.status}</b></h1>
                            </div>
                          </div>
                        })}
                      ...
                      :
                      <div className="flex justify-center items-center">
                        <NoInfoCard header={'No bids available'} message={'All bids will be displayed here'}/>
                      </div>
                    }
                  </div>
                }
